...
int detect_io_uring_write(struct pt_regs *ctx) {
    struct io_uring_sqe *sqe = (struct io_uring_sqe *)PT_REGS_PARM1(ctx);
    u64 tgid = bpf_get_current_pid_tgid();
    u32 pid = tgid >> 32;
    ...
    keep_listen = listen_map.lookup(&key);
    if (!keep_listen) {
    ...
    void *ptr = (void *)sqe->addr;
    size_t count = (size_t)sqe->len;
    if (count <= 0 || count > MAX_LOG_SIZE) {
        return 0;
    }
    ...
    int str = *start;
    bpf_probe_read(&block_buffer, sizeof(char) * LOG_BLOCK_SIZE, ptr + str);
...
// int main() {
//     // 注册 io_uring 写入拦截
//     bpf_trace_printk("Registering io_uring write hook\n");
